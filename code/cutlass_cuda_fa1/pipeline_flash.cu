...
__device__ __forceinline__ void apply_rescaling_in_frag(
    ...
    // 这种映射是针对 Volta/Ampere/Hopper 上 FP32 accumulator 的经验映射：
    // Thread 0-3  持有 Row 0 和 Row 8
    // Thread 4-7  持有 Row 1 和 Row 9
    // ...
    // Thread 28-31 持有 Row 7 和 Row 15
    // 每个线程有 8 个元素：前 4 属于 row_group1，后 4 属于 row_group2
...
// ==================== 辅助函数：加载第 tile_idx 个 K/V tile 到指定 smem ====================
template<int HEAD_DIM>
__device__ __forceinline__ void load_kv_tile(
    ...
    int total   = k_size * HEAD_DIM;

    for (int idx = tid; idx < total; idx += blockDim.x) {
        int r = idx / HEAD_DIM;
        int c = idx % HEAD_DIM;

        smem_K_tile[idx] = K_base[(k_start + r) * stride_s + c];
        smem_V_tile[idx] = V_base[(k_start + r) * stride_s + c];
    }
}
...
__global__ void flash_attn_cutlass_kernel(
    ...
    half* smem_K1 = smem_K0 + kv_tile_elems;  // 第二块紧挨着第一块元素
    ...
    using half2 = __half2;
    int vec = HEAD_DIM / 2;  // 每行有 vec 个 half2
    for (int idx = tid; idx < q_size * vec; idx += blockDim.x) {
        int r  = idx / vec;
        int c2 = idx % vec;

        const half2* src = reinterpret_cast<const half2*>(
            Q_base + (q_start + r) * stride_s
        );
        half2* dst = reinterpret_cast<half2*>(smem_Q + r * HEAD_DIM);
        dst[c2] = src[c2];
    }
    __syncthreads();
    ...
    // ================= MAIN LOOP with K/V double-buffer pipeline =================

    // 1) 预加载 tile 0 到 stage 0
    if (num_kv_tiles > 0) {
        load_kv_tile<HEAD_DIM>(
    ...
    __syncthreads();  // 确保 tile0 ready

    int stage = 0;    // 当前使用的 buffer：0 或 1
    ...
    for (int kv_tile_idx = 0; kv_tile_idx < num_kv_tiles; ++kv_tile_idx) {
        ...
        // 当前 tile 的 k-start/k-size
        int k_start = kv_tile_idx * kTileN;
        int k_end   = min(k_start + kTileN, seq_len);
        int k_size  = k_end - k_start;

        // 2) 预取下一 tile 到另一个 stage（和 compute 形成“软件流水线”）
        int next_tile  = kv_tile_idx + 1;
        int next_stage = stage ^ 1;  // 0->1, 1->0
        ...
        if (next_tile < num_kv_tiles) {
            ...
            // 不立刻 sync，让硬件有机会重叠 load & compute
        }

        // 3) 在当前 tile 上做 QK^T + softmax + PV
        if (m_valid > 0) {
        ...
        // 4) 等待刚才发出的下一 tile load 完成，然后切换 stage
        __syncthreads();
        stage = next_stage;
    }
