...
Here we count only matrix multiplication for TFLOPS calculation, since $Softmax$ involves much fewer FLOPS. For a matrix $A$ of size $m \times n$ and a matrix $B$ of size $n \times p$,  the multiplication $A \times B$ requires $m \times n \times p$ multiplications and $m \times n \times p$ additions, totaling $2mnp$ FLOPs.
...
\begin{table}[htbp]
\centering
\caption{Flash Attention Throughput Comparison (TFLOPS and Speedup vs. Naive)}
...
\textbf{Config (B, H, N, D)} & \multicolumn{1}{c}{\textbf{Naived}} & \multicolumn{1}{c}{\textbf{TFLOPS}} & \multicolumn{1}{c}{\textbf{Speedup}} & \multicolumn{1}{c}{\textbf{TFLOPS}} & \multicolumn{1}{c}{\textbf{Speedup}} & \multicolumn{1}{c}{\textbf{TFLOPS}} & \multicolumn{1}{c}{\textbf{Speedup}} \\
\midrule
